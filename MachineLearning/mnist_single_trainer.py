...
class Trainer:
    def __init__(self):
        ...
        self.y_ = tf.placeholder(tf.int64, [None, 1001], name='y_')
    ...
    def load_training_dataset(self, path='data/MNIST_synthetic.h5'):
        h5f = h5py.File(path, 'r')

        self.X_train = h5f['train_dataset'][:]
        self.y_train = h5f['train_labels'][:]
        self.y_val = h5f['valid_labels'][:]
        self.X_val = h5f['valid_dataset'][:]
        self.X_test = h5f['test_dataset'][:]
        self.y_test = h5f['test_labels'][:]
    ...
    def convDeepnn(self, x):
        ...
        W_fc1 = self.weight_variable([32 * 32 * 64, 1024])
        ...
        W_fc2 = self.weight_variable([1024, 1001])
        b_fc2 = self.bias_variable([1001])
    ...
    def run_training(self):
        ...
        for i in range(200000):
            offset = (i * self.batch_size) % (self.y_train.shape[0] - self.batch_size)
            batch_data = self.X_train[offset:(offset + self.batch_size), :]
            batch_labels = self.y_train[offset:(offset + self.batch_size), :]

            if i % 100 == 0:
                train_accuracy = accuracy.eval(feed_dict={
                    self.x: batch_data, self.y_: batch_labels, keep_prob: 1.0})

                print('step {}, training accuracy {}'.format(i, train_accuracy))

            train_step.run(feed_dict={self.x: batch_data, self.y_: batch_labels, keep_prob: 0.5})

        print('test accuracy %g' % accuracy.eval(feed_dict={
            self.x: self.X_val[:1000], self.y_: self.y_val[:1000], keep_prob: 1.0}))
